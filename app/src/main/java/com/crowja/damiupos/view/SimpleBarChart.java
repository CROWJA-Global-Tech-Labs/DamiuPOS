package com.crowja.damiupos.view;
...
public class SimpleBarChart extends View {
    ...
    public void setData(List<BarData> data) {
        ...
        // requestLayout() supaya onMeasure dipanggil ulang — width bisa berubah
        // mengikuti jumlah bar saat di dalam HorizontalScrollView.
        requestLayout();
        invalidate();
    }

    /**
     * Wajib di-override: saat view berada di dalam {@link android.widget.HorizontalScrollView},
     * parent mengukur child dengan {@code MeasureSpec.UNSPECIFIED} untuk lebar.
     * Tanpa override, {@link View#onMeasure} default mengembalikan lebar = 0
     * (suggested minimum width), sehingga chart tidak terlihat sama sekali.
     *
     * <p>Di sini: kalau lebar EXACTLY (mis. match_parent di LinearLayout) → pakai
     * apa adanya. Kalau UNSPECIFIED/AT_MOST (di dalam scroll view) → pakai lebar
     * eksplisit dari LayoutParams, atau hitung dari jumlah bar (min 80dp/bar).
     */
    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            int lpWidth = getLayoutParams() != null ? getLayoutParams().width : 0;
            int desired;
            if (lpWidth > 0) {
                desired = lpWidth;
            } else {
                int perBar = (int) (80 * getResources().getDisplayMetrics().density);
                int minWidth = (int) (320 * getResources().getDisplayMetrics().density);
                desired = Math.max(data.size() * perBar, minWidth);
            }
